// Into You - Falling Hearts
// by Martijn Steinrucken aka BigWings - 2018
...
vec3 qmulv(vec4 q, vec3 p) {
    return qmulq(q, qmulq(vec4(p, .0), qinv(q))).xyz;
}
...
void main() {
    ...
    float focalPlane = 0.4; // Fixed focal plane (was mouse controlled)
    ...
    for (float i = 0.; i < 1.; i += (1./NUM_HEARTS)) {
        ...
        float blur = mix(.03, .35, S(.0, .4, abs(focalPlane - i)));
